...
const IssueDetails = () => {
  ...
  const handleBoost = async () => {
    ...
    try {
      const res = await axios.patch(`http://localhost:3000/issues/boost/${id}`, { boosted: true });
      setIssue((prev) => ({ ...prev, ...res.data }));
    } catch {
      alert("Boost failed");
    }
        ...
        <div className="card-body">
            ...
            <div className="mt-6">
              ...
              <div className="flex flex-col-reverse gap-3">
                {issue.timeline.slice().reverse().map((t, idx) => (
                  <div key={idx} className="p-3 border-l-4 border-blue-500 bg-gray-50 rounded">
                    <div className="flex justify-between text-sm">
                      <span className={`badge ${
                        t.status === "Resolved" ? "badge-success" :
                        t.status === "In-Progress" ? "badge-warning" :
                        t.status === "Boosted" ? "badge-success" :
                        t.status === "Closed" ? "badge-error" :
                        "badge-primary"
                      }`}>
                        {t.status}
                      </span>
                      <span className="text-gray-400">{new Date(t.date).toLocaleString()}</span>
                    </div>
          ...
          <div className="mt-8 flex gap-4">
            {user?.email === issue.postedBy && issue.status === "Pending" && (
              <Link to={`/edit-issue/${id}`} className="btn btn-warning">✏️ Edit</Link>
            )}
