...
const AllIssues = () => {
  ...
  const fetchIssues = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(res.data) ? res.data.filter((i) => i?._id) : [];
      setIssues(list);
    } catch (err) {
  ...
  const handleUpvote = async (issue) => {
    ...
    setIssues((prev) =>
      prev.map((i) =>
        i._id === issue._id
          ? { ...i, upvotes: (i.upvotes || 0) + 1, upvotedUsers: [...(i.upvotedUsers || []), user.email] }
          : i
      )
    );

    try {
      const res = await axiosSecure.put(`/issues/${issue._id}/upvote`, { userEmail: user.email });
  ...
  const filtered = useMemo(() => {
    ...
    if (priority) data = data.filter((i) => i.priority?.toLowerCase() === priority.toLowerCase());
    data.sort((a, b) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10">
      <h2 className="text-3xl font-bold text-center mb-8 text-primary">All Issues</h2>

      {/* Filters */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <input type="text" placeholder="Search issue..." className="input input-bordered w-full" value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {currentIssues.length ? (
          currentIssues.map((issue) => {
            const alreadyUpvoted = issue.upvotedUsers?.includes(user?.email);
            return (
              <div key={issue._id} className="group card bg-white rounded-xl overflow-hidden shadow-md transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 hover:scale-[1.03]">
                <figure className="overflow-hidden">
                  <img src={issue.image || "https://via.placeholder.com/600x350?text=No+Image"} alt={issue.title} className="h-48 w-full object-cover transition-transform duration-300 group-hover:scale-110" />
                </figure>
                <div className="card-body flex flex-col">
                  ...
                  <div className="flex gap-2 mt-auto flex-wrap">
                    <button onClick={() => handleUpvote(issue)} className={`btn btn-sm transition-all duration-300 ${alreadyUpvoted ? "btn-success text-white" : "btn-outline"}`} disabled={issue.postedBy?.email === user?.email}>
                      <AiFillLike className="text-yellow-400 text-xl" /> {issue.upvotes || 0}
                    </button>
                    <Link to={`/issues/${issue._id}`} className="btn btn-sm btn-primary transition-all duration-300 group-hover:scale-105">View</Link>
                  </div>
                </div>
              </div>
            );
